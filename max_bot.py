...
# --- НАСТРОЙКИ ---
RSS_URL = "https://bg.raindrop.io/rss/public/74027357" # Прямая RSS-ссылка
PROCESSED_FILE = "processed_max.txt"
OUTPUT_DIR = "max_posts"
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY")
POSTS_PER_RUN = 1 # <--- ЛИМИТ ПОСТОВ ЗА ОДИН ЗАПУСК
...
def main():
    ...
    count = 0 # Счетчик написанных постов
    ...
    for entry in feed.entries:
        ...
        if article_text:
            ...
            if count >= POSTS_PER_RUN:
                print(f"🛑 Достигнут лимит {POSTS_PER_RUN} постов за один запуск. Идем отдыхать.")
                break
