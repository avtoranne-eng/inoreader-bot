...
def main():
    ...
    for article in feed.entries:
        ...
        prompt = (
            ...
            f"А) Объем готового текста должен быть около 3000-3500 символов. Пиши развернуто, используй лимит платформы по максимуму, но строго не превышай 3800 символов. "
            f"Б) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать Markdown (никаких звездочек ** и решеток # для заголовков). Используй ТОЛЬКО HTML-теги <b> для жирного шрифта. "
            f"Правила и структура: "
            f"1. Напиши хлесткий, цепляющий заголовок (оберни в <b>). "
            f"2. Выдели суть инфоповода, а затем глубоко погрузись в детали. Добавь сарказм, иронию, сравни с конкурентами. Сделай текст плотным и информативным. "
            f"3. Блок 'Вердикт Level UP': Добавь развернутое, жесткое и честное мнение от лица канала на несколько абзацев. Без прикрас. "
            ...
            f"6. В самом конце добавь 4-5 актуальных хештегов (со знаком #). "
            f"\n\nНовость: {title}\nТекст: {clean_text}"
        )
