...
def main():
    ...
    for article in feed.entries:
        ...
        prompt = (
            ...
            f"Твоя задача: Написать захватывающую, глубокую и очень длинную лонгрид-статью (строго от 5000 символов), которая удержит читателя до последней строчки и привлечет органический поисковый трафик. "
            f"КРИТИЧЕСКОЕ ПРАВИЛО ФОРМАТИРОВАНИЯ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать Markdown (никаких звездочек **, решеток ### или подчеркиваний). Выделяй текст ТОЛЬКО с помощью HTML-тегов: <b>жирный текст</b> и <i>курсив</i>. "
            f"Правила и структура: "
            f"1. Заголовок: Сформулируй жестко под конкретные поисковые запросы геймеров (например, вместо 'Новая игра' пиши 'Скачать лучшие моды на графику для...'). Заголовок оберни в тег <b>. "
            f"2. Приветствие: Начни с живого приветствия аудитории канала Level UP. "
            f"3. SEO и трафик: Органично вшивай нужные ключевые слова в текст, чтобы нас находили через поиск Яндекса и Google. Текст должен быть поисково привлекательным, но читаться естественно. "
            f"4. Глубина и объем: Раскрой инфоповод максимально широко. Добавь предысторию серии, проанализируй влияние новости на индустрию, используй сторителлинг и литературный стиль. Не скупись на детали — нужен большой объем. "
            ...
            f"6. Вовлечение: Заверши лонгрид мощным призывом к читателям спуститься в комментарии и задай им конкретный, провокационный вопрос по теме. "
            f"\n\nНовость: {title}\nТекст: {clean_text}"
        )
