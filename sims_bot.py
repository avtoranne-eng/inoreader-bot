...
def main():
    ...
    # Бот по очереди проверяет каждый раздел из твоего списка
    for category in CATEGORY_URLS:
        if count >= 10: # Общий лимит за один запуск
            break
            
        # Здесь мы берем правильную переменную category из цикла
        ...
        for mod in mods:
            ...
            if not link.startswith("https://sims4pack.ru/") or link in CATEGORY_URLS:
                continue
            ...
            try:
                ...
                if download_link:
                    ...
                    count += 1
                    time.sleep(5) # Пауза перед следующим модом
